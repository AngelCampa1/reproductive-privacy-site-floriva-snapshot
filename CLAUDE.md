...
- The design system is **Bone & Berry** — the mobile app's editorial design language, ported to web. Tokens live in `src/styles/tokens.css` and are sourced from the app repo's `floriva-app/src/theme/tokens.ts`; the app is the source of truth, and this site's visual language must stay aligned with it.
- Core tokens: `--canvas` bone (#F4ECE0) as the base surface, `--paper` (#FBF5EB) for cards at `radius-card` (24px), `--ink` (#1A1410) for text, a single `--berry` accent (#923030, no secondary rose/gold), `--moss` (#6E8E6B) for success/secondary states, and `--rule` hairline (#D9CCBB) for borders/dividers.
- Typography: `--font-display` is Newsreader (serif, with italic accents for emphasis), `--font-body` is Inter Tight, `--font-mono` is JetBrains Mono for numerals/stats. All fonts are self-hosted — do not add Google Fonts `<link>` tags or other externally-hosted font requests.
- Shadows and elevation stay flat and minimal (`--shadow-soft` / `--shadow-strong`); avoid heavy drop shadows, glassmorphism, or translucent chrome (no stray `backdrop-filter` glass panels) outside of deliberate, narrowly-scoped exceptions.
- Do not drift back into generic startup UI patterns, default Vite/Tailwind aesthetics, or the retired sage/rose/gold oklch palette.
